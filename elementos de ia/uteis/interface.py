...
from board_states.board_manager import save_board_state
...
for ball_value, ball_name in ball_types.items():
    image_path = f"bolas/{ball_name}.jpg"
    try:
...
def show_rules():
    ...
    rules_text = [
        ...
        "",
        "",
        "Como se ganha este jogo?",  # Line 3
        "Para ganhar, organize todas as bolas,",  # Line 4
        "com uma prateleira para cada desporto.",
        "",
        "Quais os possíveis movimentos?",  # Line 7
        "Só pode mover bolas para prateleiras vazias",  # Line 8
        "ou para uma prateleira onde a bola mais",
        "à direita é igual à bola que está a mover.",
        "",
    ...
    for i, line in enumerate(rules_text):
        ...
        elif i == 3 or i == 7:  # Question lines (3 and 7)
        ...
        if i == 3 or i == 7:  # After question lines (3 and 7)
            y_offset += HEADER_SPACING
        elif i == 1 or i == 6:  # After empty lines
...
def draw_game_screen(board, shelves, level, message="", selected_shelf=None, selected_ball=None, possible_moves_list=None):
    ...
    if possible_moves_list and selected_shelf is not None:
        for move_idx in possible_moves_list:
            if move_idx < len(shelves):  # Ensure move_idx is valid
                shelf = shelves[move_idx]
...
def start_game(level):
    ...
    while level_running:
        ...
        if check_win(board, level):
            message = "PARABÉNS! GANHASTE!"
            game_won = True
            save_board_state(board, level)
        
        draw_game_screen(board, shelves, level, message, selected_shelf, selected_ball, possible_moves_list)
...
def show_jogar():
    ...
    # Create overlay surface (same as rules screen)
    overlay = pygame.Surface((largura, altura), pygame.SRCALPHA)
    ...
    while level_running:
        ...
        # Draw background with overlay (same as rules screen)
        tela.blit(fundo, (0, 0))
        tela.blit(overlay, (0, 0))
        
        # Draw title
        title_text = title_in_rules.render("ESCOLHA O NÍVEL", True, really_light_blue)
